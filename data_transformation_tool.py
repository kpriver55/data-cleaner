...
class DataTransformationTool:
    ...
    def handle_missing_values(
        ...
        categorical_columns = [
            ...
            for col in columns
            if col in df_clean.columns
            and not pd.api.types.is_numeric_dtype(df_clean[col])
            and df_clean[col].isna().any()
        ]

        # Handle numeric columns
        if numeric_columns:
            df_clean, num_processed = self._apply_numeric_strategy(
                df_clean, numeric_strategy, numeric_columns
            )
            processed_columns.extend(num_processed)

        # Handle categorical columns
        if categorical_columns:
            df_clean, cat_processed = self._apply_categorical_strategy(
                df_clean, categorical_strategy, categorical_columns, custom_values
            )
    ...
    def _apply_numeric_strategy(
        self, df: pd.DataFrame, strategy: str, columns: List[str]
        ...
        for col in columns:
            if strategy == "mean":
                ...
                fill_value = df_clean[col].median()
                print(
                    f"Warning: Unknown numeric strategy '{strategy}', defaulting to median for column '{col}'"
                )
    ...
    def _apply_categorical_strategy(
        ...
        for col in columns:
            if strategy == "most_frequent" or strategy == "mode":
                fill_value = (
                    df_clean[col].mode().iloc[0] if len(df_clean[col].mode()) > 0 else "Unknown"
                )
                ...
                print(
                    f"Warning: Unknown categorical strategy '{strategy}', defaulting to most_frequent for column '{col}'"
                )
                ...
                if col in df_clean.columns and df_clean[col].isna().any():
                    if pd.api.types.is_numeric_dtype(df_clean[col]):
                        ...
                        if strategy in ["mean", "median"]:
                            ...
                            print(
                                f"Note: Adapted strategy from '{strategy}' to 'most_frequent' for non-numerical column '{col}'"
                            )
                        else:  # most_frequent
                            fill_value = (
                                df_clean[col].mode().iloc[0]
    ...
    def normalize_scale_data(
        ...
        self.log_transformation(
            ...
            {"method": method, "columns_processed": numeric_columns, "scaler_name": scaler_name},
        )
    ...
    def clean_text_columns(
        ...
        df: pd.DataFrame,
        columns: Optional[List[str]] = None,
        operations: List[str] = ["strip", "lower", "remove_special"],
    ) -> pd.DataFrame:
        """
        Clean text columns.
        ...
        self.log_transformation(
            "clean_text_columns", {"columns_processed": text_columns, "operations": operations}
        )

        return df_clean

    def convert_data_types(
        self, df: pd.DataFrame, type_conversions: Dict[str, str]
    ) -> pd.DataFrame:
        """
        ...
        for col, target_type in type_conversions.items():
            ...
            try:
                if target_type.lower() in ["int", "int64", "integer"]:
                    df_converted[col] = pd.to_numeric(df_converted[col], errors="coerce").astype(
                        "Int64"
                    )
    ...
    def create_derived_features(
        self, df: pd.DataFrame, feature_definitions: Dict[str, str]
    ) -> pd.DataFrame:
        """
    ...
    def filter_rows(self, df: pd.DataFrame, conditions: Dict[str, Any]) -> pd.DataFrame:
        """
            ...
            for operator, value in condition_dict.items():
                ...
                if operator == "==":
                    ...
                    df_filtered = df_filtered[
                        df_filtered[col].astype(str).str.contains(str(value), na=False)
                    ]
        ...
        self.log_transformation(
            "filter_rows", {"applied_filters": applied_filters, "final_row_count": len(df_filtered)}
        )
...
if __name__ == "__main__":
    ...
    sample_data = pd.DataFrame(
        {
            ...
            "salary": np.concatenate(
                [np.random.normal(50000, 15000, 995), [200000, 300000, 10000, 400000, 500000]]
            ),
    ...
    scaled_data = transform_tool.normalize_scale_data(
        encoded_data, columns=["age", "salary", "score"]
    )
