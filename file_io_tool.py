...
class FileIOTool:
    ...
    def read_spreadsheet(
        ...
        try:
            ...
            print(
                f"Successfully loaded {len(df)} rows and {len(df.columns)} columns from {file_path}"
            )
            return df
    ...
    def write_spreadsheet(self, df: pd.DataFrame, file_path: str, **kwargs) -> bool:
        """
        ...
        try:
            if file_ext == ".csv":
                default_csv_params = {"index": False, "encoding": "utf-8"}
                default_csv_params.update(kwargs)
    ...
    def write_json(self, data: Union[Dict, List], file_path: str, indent: int = 2) -> bool:
        """
    ...
    def write_text(self, content: str, file_path: str, encoding: str = "utf-8") -> bool:
        """
    ...
    def get_file_info(self, file_path: str) -> Dict[str, Any]:
        ...
        info = {
            ...
            "modified": pd.Timestamp.fromtimestamp(file_path.stat().st_mtime).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
        }
...
if __name__ == "__main__":
    ...
    io_tool.write_markdown(sample_data, "output/data_report.md", title="Sample Data Analysis")
