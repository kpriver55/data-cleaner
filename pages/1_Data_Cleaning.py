...
def main():
            ...
            try:
                ...
                if provider_choice == "Local (Ollama)":
                    st.session_state.llm_config.configure_ollama(
                        model=model_choice, api_base=ollama_base
                    )
                elif provider_choice == "OpenAI":
                ...
                st.success(
                    f"✅ Agent initialized with {config_info['provider']} / {config_info['model']}"
                )
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📁 Upload Data", "📊 Data Overview", "🤖 AI Cleaning", "📈 Results"]
    )
                ...
                if start_cleaning:
                    with st.spinner("AI is analyzing your data..."):
                        ...
                        try:
                            ...
                            cleaned_df = st.session_state.cleaning_agent.io_tool.read_spreadsheet(
                                results["output_file"]
                            )
                            ...
                            if results["operations_log"]:
                                with st.expander("📋 Operations Performed"):
                                    for i, op in enumerate(results["operations_log"], 1):
                                        ...
                                        st.write(
                                            f"Impact: {op['rows_before']} → {op['rows_after']} rows"
                                        )
                                        st.write("---")
    ...
    with tab4:
        if st.session_state.cleaned_df is not None:
            ...
            base_name = (
                Path(st.session_state.original_filename).stem
            ...
            with col3:
                ...
                try:
                    if (
                        ...
                        and "report_file" in st.session_state.cleaning_results
                    ):
